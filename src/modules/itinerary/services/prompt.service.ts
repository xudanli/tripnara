...
Please start generating JSON:`;
  ...
  private buildLocationGenerationUserPromptZh(params: {
    ...
    return `你是一名崇尚极简主义的旅行情报专家。请根据以下信息，生成**精简、直观、高可用**的地点情报，专门用于移动端卡片展示。
...
1. **拒绝废话**：去除所有修饰词、连接词和客套话（如"我们建议"、"大约"、"位于"）。

2. **拒绝长句**：禁止使用完整句子，必须使用"短语"或"关键词"。

3. **高密度信息**：多条信息用分号"；"分隔，关键信息（如站名、时间、价格）前置。

4. **字数限制**：严格遵守每个字段的字数上限。

【字段生成要求】

1. **名称**：仅保留官方标准名称，不要别名或后缀。（<10字）

2. **地址**：仅保留街道名+门牌号，或核心地标名。去除行政区划/邮编。（<15字）

3. **交通**：
   - 格式：[方式] 关键站名/路线 (步行耗时)
   - 示例：地铁1号线 Palais Royal站(步3分)；公交21路 Louvre站；自驾至Indigo停车场。
   - 限制：<40字
...
5. **门票**：
   - 格式：成人XXX；儿童XXX；[需预约/免票]。
   - 限制：<20字

6. **游览/避坑**：
   - 提炼3个最关键的行动点。
   - 示例：必须官网提前预约；馆内禁闪光灯；背包需前置。
   - 限制：<30字

7. **周边**：列出最近的2个核心地标/便利设施。
   - 限制：<10字
...

{
  "chineseName": "标准中文名",
  "localName": "当地语言名",
  "chineseAddress": "极简中文地址",
  "localAddress": "极简当地地址",
  "transportInfo": "极简交通指引",
  "openingHours": "极简时间表",
  "ticketPrice": "极简票价规则",
  "visitTips": "3条核心避坑/建议",
  "nearbyAttractions": "2个周边地标",
  "contactInfo": "官网短链接",
  "category": "活动类型",
  "rating": 评分(1-5),
  "visitDuration": "时长(如: 3h)",
  "bestTimeToVisit": "最佳时段(如: 周五晚)",
  "accessibility": "核心设施(如: 有电梯)",
  "dressingTips": "核心穿搭关键词",
  "culturalTips": "核心禁忌(如: 禁三脚架)",
  "bookingInfo": "极简预订策略"
}`;
  }
...
> 🚨 **紧急求助**：(当地报警/急救电话)`;
   ...
   * 构建本地生活指南生成系统提示词
   */
  buildLocalEssentialsSystemMessage(language: string = 'zh-CN'): string {
    const isEnglish = language === 'en-US' || language === 'en';
    
    if (isEnglish) {
      return `You are a professional travel information assistant, skilled at providing accurate destination practical information. Please always return data in JSON format.`;
    }
    
    return `你是一个专业的旅行信息助手，擅长提供准确的目的地实用信息。请始终以JSON格式返回数据。`;
  }

  /**
   * 构建本地生活指南生成用户提示词
   */
  buildLocalEssentialsUserPrompt(params: {
    destination: string;
    language?: string;
  }): string {
    const language = params.language || 'zh-CN';
    ...
    if (isEnglish) {
      return `Please provide the following practical information for destination **${params.destination}** and return it in JSON format:
...
Please ensure the return is valid JSON format, all fields are string type.`;
    ...
    return `请为目的地 **${params.destination}** 提供以下实用信息，并以JSON格式返回：
