    ...
    "activities": [
      {
        ...
        "notes": "...",
        "time": "1h",
        "location": { "lat": 63.8804, "lng": -22.4493 },
        "type": "景点/美食/户外/文化/市集"
      }
    ]
  }
...
1. **"location" 字段必须是一个包含 "lat" 和 "lng" 的 JSON 对象，绝对不能是字符串！**
   - ❌ 错误示例: "location": "蓝湖温泉"
   - ❌ 错误示例: "location": "Reykjavik, Iceland"
   - ✅ 正确示例: "location": { "lat": 63.8804, "lng": -22.4493 }
   - ✅ 正确示例: "location": { "lat": 64.1265, "lng": -21.8174 }

2. **如果你不知道确切坐标，请务必估算一个大概的经纬度，绝对不要返回字符串！**
   - 可以使用地图服务或常识估算大致位置
   - 即使坐标不够精确，也必须提供数字格式的经纬度对象
   - 这是硬性要求，违反此要求将导致解析失败
