...
### 6. 预算管理接口（/api/v1/journeys/:journeyId/expenses）

- ✅ `GET /api/v1/journeys/:journeyId/expenses` - 获取支出列表（支持按分类、日期、付款人筛选）
...
- ✅ `PATCH /api/v1/journeys/:journeyId` - 更新预算总额（使用现有接口的 totalCost 字段）

**功能特性：**
- 支持多种货币（currencyCode）
- 支持支出分类（交通、住宿、餐饮、景点、购物、其他）
...
- ✅ `GET /api/v1/destinations/:id/weather` - 天气信息（可选，占位符实现，待配置 API）
...
- **已完成：** 33 个接口（行程 CRUD + 行程模版基础操作 + 行程 v1 基础接口 + 细粒度操作接口 + 预算管理接口）
- **未完成：** 约 60+ 个接口
