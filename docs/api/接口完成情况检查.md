...
### 1. 模板管理（Template）- 细粒度接口

根据 `src/docs/接口清单.md`，以下接口尚未实现：
...
**细粒度操作接口：**
- ❌ `GET /api/v1/templates/:templateId/days` - 获取模板的所有天数
- ❌ `POST /api/v1/templates/:templateId/days` - 为模板添加天数
- ❌ `PATCH /api/v1/templates/:templateId/days/:dayId` - 更新指定天数
- ❌ `DELETE /api/v1/templates/:templateId/days/:dayId` - 删除指定天数
- ❌ `GET /api/v1/templates/:templateId/days/:dayId/slots` - 获取指定天数的所有时间段
- ❌ `POST /api/v1/templates/:templateId/days/:dayId/slots` - 为指定天数添加时间段
- ❌ `PATCH /api/v1/templates/:templateId/days/:dayId/slots/:slotId` - 更新指定时间段
- ❌ `DELETE /api/v1/templates/:templateId/days/:dayId/slots/:slotId` - 删除指定时间段

**说明：** 当前实现的是 `/api/v1/itineraries`（行程模版），但接口清单要求的是 `/api/v1/templates`。这两个可能是不同的概念，需要确认。
...
- ❌ `GET /api/v1/journeys/:journeyId/export` - 导出行程（PDF/ICS 等）
...
**说明：** 当前实现的是 `/api/itinerary`（旧版接口），但接口清单要求的是 `/api/v1/journeys`（新版 v1 接口）。需要确认是否需要迁移或同时支持。
...
**说明：** 当前只有 `POST /api/itinerary/generate`（一次性生成），缺少分阶段生成和状态查询功能。
...
- ❌ `POST /api/v1/preparation-profiles` - 创建任务模板（仅管理员）
...
1. **接口路径差异：** 当前实现的接口路径与接口清单中的路径不完全一致：
   - 当前：`/api/itinerary` 和 `/api/v1/itineraries`
   - 清单：`/api/v1/journeys` 和 `/api/v1/templates`
