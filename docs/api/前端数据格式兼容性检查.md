...
- ✅ 支持 `itineraryData.days` 数组（包含 `timeSlots`）
...
### 2. ❌ PATCH /api/itinerary/:id (更新行程)

**兼容性：不兼容**

**问题：**
- ❌ `UpdateItineraryRequestDto` 只支持简单字段更新（destination, startDate, days, summary, totalCost, preferences, status）
...
修改 `UpdateItineraryRequestDto`，添加对 `itineraryData` 的支持：

```typescript
export class UpdateItineraryRequestDto {
  // ... 现有字段 ...
  
  @ApiPropertyOptional({ description: '完整的行程数据（前端格式）' })
  @IsOptional()
  @ValidateNested()
...
然后在 `updateItinerary` 方法中处理 `itineraryData`，更新 days 和 activities。
